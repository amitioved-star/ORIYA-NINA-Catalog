...
export default function LandingPage({ type = '/dress-rental-harish' }) {
  ...
  const sourceKey = `landing${type.replace(/\W+/g, '_')}`
    ...
    <div className="bg-cream-50">
      ...
      <section className="bg-gradient-to-b from-cream-100 to-cream-50 px-4 py-16 text-center">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          ...
          <a
            href={buildWhatsappUrl(`שלום, אני מעוניינת בפרטים על ${data.h1}`)}
            target="_blank"
            rel="noopener noreferrer"
            onClick={() => { trackWhatsApp(`${sourceKey}_hero`); trackBookMeasurement(`${sourceKey}_hero`) }}
            className="btn-outline"
            aria-label={`קביעת מדידה בוואטסאפ — ${data.h1}`}
          >
            קביעת מדידה
          </a>
        </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-4 py-14 md:grid-cols-2">
        ...
        <div className="rounded-3xl bg-white p-8 shadow-sm border border-cream-200">
          ...
          <a
            href={buildWhatsappUrl('שלום, אשמח לבדוק זמינות למדידה בחריש')}
            target="_blank"
            rel="noopener noreferrer"
            onClick={() => trackWhatsApp(`${sourceKey}_area`)}
